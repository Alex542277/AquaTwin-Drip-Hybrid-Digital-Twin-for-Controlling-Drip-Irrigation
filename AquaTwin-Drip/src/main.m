function main_good(culture,typeSol,pas_heure,JourJulien,lat,lon)
    ...
    while tempsSimule < dureeSimulation
        ...
        if k==1
            ...
            % Une seule figure avec deux sous-graphiques
            figure(100);
            subplot(2,1,1);
            ...
            [SH,Pt]=TraceTeneurEnEauRacinaireStresseHydriqueEtPotentielHydrique_(lat,lon,culture,typeSol,Tmax+3600,solution,Tmax,heure,alpha_vg,n_vg, m_vg,theta_s, theta_r,k_s);
            Theta_r=Theta_root;
                
        end

        % Gestion de la date jusqu'au rendement
        next=datetime('today');
        if next==today
        ...
        % PAUSE D'UNE HEURE-SYSTEME EN TEMPS REEL
