function [solution, Erreur,Tmax,V]=DDFVRichardIrrigation(psi_old,J,culture,typeSol,lat,lon)
    ...
    [Tmax,V]=TempsEtVolumeEauNecessaireIrrigation(q_irr,total_dof,J,culture,typeSol,lat,lon);
    [max_iter,tol,t]=valorsForSimulation(mean(Tmax));
    ...
    psi_current = psi_old;
    K=TenseurSol(typeSol);
    D = r*K;  % Tenseur de permeabilite
    ...
    fprintf('Volume Eau Necesaire %d', V);
    fprintf('Temps Irrigation: %d', mean(Tmax));
    ETo=CalculEvapotranspirationJournaliere(J,culture,typeSol,lat,lon); %J : jour julien
    ...
    for nt = 1:length(t)
        ...
        fprintf('Pas de temps %d/%d, t = %.3f s\n', nt, length(t), t(nt));
     
        % Initialisation pour la boucle de Picard
        
        for iter = 1:max_iter
            
            % Reinitialisation de la matrice et du second membre cause de la
            % methode de Picard
            ...
            for i = 1:N
                for j = 1:N
                    ...
                    S_pot=Aire*Tpot*A*ro(xc,yc)*phi(t(nt))*10^(-3)/dt;
                    S_reel=S_pot(idx)*alpha_(psi_current);
                    F(idx) = h^2 *S_reel/C_val;
                end
            end
            ...
            for i = 0:N
                for j = 0:N
                    ...
                    if i > 0 && i < N && j > 0 && j < N
                        ...
                        S_pot=A*Tpot*Aire*ro(xc,yc)*phi(t(nt))*10^(-3)/dt;
                        S_reel=S_pot(idx)*alpha_(psi_current);
                        F(idx_dual) = h^2 *S_reel/C_val;
                    end
                end
            end
            ...
            psi_new=(exp(-nt*A)*(psi_old-psi_inc))+psi_inc;
            ...
            if erreur <tol && iter< max_iter
                fprintf('Convergence a l''iteration %d, erreur = %.2e\n', iter, erreur);
                psi_current = psi_new;
                break;
            else
